  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setApiError(
        getErrorMessage(
          err,
          `Unable to sign in right now. Please try again, or contact ${SUPPORT_EMAIL} if the problem persists.`
        )
      )
    } finally {
      ...
      <div className="hidden lg:flex w-[420px] shrink-0 bg-[#2D5240] flex-col justify-center p-12">
        ...
        <div>
          ...
          <p className="text-white/60 text-sm leading-relaxed">
            Sign in to view your transactions, check group balances, and see your
            AI-powered insights.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm p-10">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            ...
            <div className="flex justify-end mb-6">
              <Link
                to="/forgot-password"
                className="text-sm text-[#3D6B4F] font-medium hover:underline"
              >
                Forgot password?
              </Link>
            </div>
